...
static void vm_area_check_alignment (struct vm_area *area) {
  if (area->va % PAGE_SIZE != 0) {
    panic("vm_area: misaligned va %p", area->va);
  }
}
...
static void vm_area_destroy (struct vm_area *area) {
  void *pa;
  list_foreach(pa, &area->pages) {
    free_pages((struct page) { .address = pa, .order = 0 });
  }
  kfree(area);
}
...
static inline bool vm_area_in (struct vm_area *area, u64 va) {
  return va >= area->va && va <= area->va + (area->n_pages << PAGE_INDEX_BITS);
}
